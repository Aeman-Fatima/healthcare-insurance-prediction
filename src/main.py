import argparse
import os
from config import Paths, SplitConfig, Columns, ModelNames
...
def parse_args():
    ...
    p.add_argument("--marketplace_path", required=True)
    p.add_argument("--complaints_path", required=True)
...
def main():
    ...
    figs_dir = os.path.join(paths.output_dir, "figures")
    tables_dir = os.path.join(paths.output_dir, "tables")
    ensure_dir(figs_dir); ensure_dir(tables_dir)

    # 1) Load & preprocess
    df = load_and_merge(paths.customers_path, paths.claims_path,
                        paths.marketplace_path, paths.complaints_path)
    ...
    models = build_models(preprocessor, cols)
